...
from schnet_nilesh_evidential import SchNetWrap as SchNet
...
class EvidentialLoss(nn.Module):
    def __init__(self, lam=0.2, epsilon=1e-4):
        super(EvidentialLoss, self).__init__()
        self.lam = lam
        self.epsilon = epsilon

    def forward(self, mu, v, alpha, beta, targets):
        #print(beta)
        ...
        L_NLL = torch.mean(nll)
        #print(torch.mean(nll))
        error = torch.abs((targets - mu))
        reg = error * (2 * v + alpha)
        L_REG = torch.mean(reg)
        #print(torch.mean(L_REG))
        loss = L_NLL + self.lam * (L_REG - self.epsilon)
...
#loss_fn = torch.nn.GaussianNLLLoss(full=False, eps=1e-06, reduction='sum')
...
for epoch in tqdm(range(train_epochs)):
    ...
    for batch in tqdm(train_loader):
        ...
        energy_alpha = graph_out[:,2]
        energy_beta = graph_out[:,3]
        energy_var = softplus(energy_var) + min_val
        energy_alpha = softplus(energy_alpha) + min_val + 1
        energy_beta = softplus(energy_beta) + min_val
        ...
        force_alpha = node_out[:,[4]]
        force_beta = node_out[:,[5]]
        force_var = softplus(force_var) + min_val
        force_alpha = softplus(force_alpha) + min_val + 1
        force_beta = softplus(force_beta) + min_val
        

        energy = energy.view(batch.y.size())
        energy_var = energy_var.view(batch.y.size())
        energy_alpha = energy_alpha.view(batch.y.size())
        energy_beta = energy_beta.view(batch.y.size())
        #energy_loss = loss_fn(batch.y, energy, energy_var)
        energy_loss = loss_fn(mu=energy,v=energy_var,alpha=energy_alpha,beta=energy_beta,targets=batch.y)
        #print(energy_loss.item()) 
        mask  = batch.fixed == 0
        natoms = batch.natoms
        ...
        force_loss = loss_fn(targets = batch.force[mask]/natoms, mu = forces[mask]/natoms, v = force_var[mask].repeat([1, 3]),alpha = force_alpha[mask].repeat([1,3]),beta=force_beta[mask].repeat([1,3]))
        ...
        if float(loss.cpu().detach().numpy()) < lowest_loss:
            torch.save(model, Path(train_dir, 'model_best_e2023.pth'))
            lowest_loss = loss
        optimizer.step()
