...
def combined_loss(y_pred, y_true, var):
    ...
    combined = 0.8 * mse_loss + 0.2 * gaussian_nll_loss
...
for epoch in tqdm(range(train_epochs)):
    ...
    for batch in tqdm(train_loader):
        ...
        if float(loss.cpu().detach().numpy()) < lowest_loss:
            torch.save(model, Path(train_dir, 'model_best_MSEg2023.pth'))
            lowest_loss = loss
        optimizer.step()
