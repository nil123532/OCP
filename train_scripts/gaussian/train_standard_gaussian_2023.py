...
for epoch in tqdm(range(train_epochs)):
    ...
    for batch in tqdm(train_loader):
        ...
        force_loss = loss_fn(batch.force[mask]/natoms, forces[mask]/natoms, force_var[mask].repeat([1, 3]))
        ...
        if float(loss.cpu().detach().numpy()) < lowest_loss:
            torch.save(model, Path(train_dir, 'model_best_g2023.pth'))
            lowest_loss = loss
        optimizer.step()
