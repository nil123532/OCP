...
class SchNetWrap(SchNet, BaseModel):
    ...
    def __init__(
        ...
        self.reduce = readout
        self.addi_node_targets = addi_node_targets
        ...
        self.lin2 = Linear(hidden_channels // 2, num_targets + 2)
        if addi_node_targets > 0:
            self.node_lin = Linear(hidden_channels, addi_node_targets)
    ...
    def _forward(self, data):
        ...
        if self.use_pbc:
            ...
            energy = scatter(h1, batch, dim=0, reduce=self.reduce)
        else:
            h = self.embedding(z)
            edge_attr = self.distance_expansion(edge_weight)
            for interaction in self.interactions:
    ...
    def forward(self, data):
        ...
        graph_out, h = self._forward(data)
        energy = graph_out[:, 0]
        if self.addi_node_targets > 0:
            node_out = self.node_lin(h)
        if self.regress_forces:
            forces = -1 * (
                torch.autograd.grad(
                    energy,
            ...
            if self.addi_node_targets > 0:
                node_out = torch.cat((forces, node_out), dim=1)
            else:
                node_out = forces
